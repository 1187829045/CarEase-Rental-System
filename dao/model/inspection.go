...
type InspectionReport struct {
	...
	Type           int8      `gorm:"column:type;comment '1:上架检测, 2:还车检测';index"`           // 检测类型
	Mileage        int64     `gorm:"column:mileage;index"`                                  // 里程数
	...
	InspectorName  string    `gorm:"column:inspector_name;type:varchar(50);index"`           // 检测人姓名
	InspectionTime time.Time `gorm:"column:inspection_time;type:datetime;index"`             // 检测时间
	Status         int8      `gorm:"column:status;default:0;comment '0:待审核, 1:已通过, 2:已拒绝';index"` // 状态
}
